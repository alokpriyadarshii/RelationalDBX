...
#
# This script builds a local version of FDB to be used by RelationalDBX when in need of a pre-release build, or any other
...
# RECORD_LAYER_HOME (Optional, defaults to ..): The top level directory of RelationalDBX
...
$MVN_HOME/mvn install:install-file \
   -Dfile=${FDB_BUILD_HOME}/packages/relationaldbx-java-${FDB_VERSION}-SNAPSHOT.jar \
   -DgroupId=org.foundationdb \
   -DartifactId=relationaldbx-java \
