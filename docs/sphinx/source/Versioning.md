# RelationalDBX Versions

## Variants

The RelationalDBX is released in two variants, depending on not Protobuf is renamed ("shaded").

* **Unshaded**: `relationaldbx-core`
* **Shaded**: `relationaldbx-core-shaded`

In the shaded versions, `com.google` classes will be renamed to `com.apple.foundationdb.record.shaded.com.google`.

Note that code generated by one version of `protoc` is generally *not* compatible with the Protobuf runtime classes from another version. For that reason, it is suggested that the adopter keep their `protoc` version aligned with the Protobuf runtime used by the RelationalDBX library.
...
```
  ...
  <artifactId>relationaldbx-core</artifactId>
  <version>4.1.+</version>
```
...
<repositories>
  <repository>
    <id>relationaldbx</id>
    <name>RelationalDBX</name>
    <url>https://ossartifacts.jfrog.io/artifactory/relationaldbx</url>
  </repository>
</repositories>
```

See: [Issue #1288](https://github.com/FoundationDB/relationaldbx/issues/1288).
...
dependencies {
  implementation "org.foundationdb:relationaldbx-core:4.1.+"
}
```
...
Classes and methods annotations using `@API` determine when they can be changed in a backwards-incompatible way or removed. See the [Javadoc](https://javadoc.io/page/org.foundationdb/relationaldbx-extensions/latest/com/apple/foundationdb/API.html) for more details.
...
* Patch branches should be named `relationaldbx-a.b.c` after the build from which they are created.

* The new patch branch should be made using the tag created for the starting build.

    ```
    git checkout -b relationaldbx-a.b.c a.b.c.0
    ```
