...
This guide covers RelationalDBX-specific JDBC features for working with complex data types like STRUCTs and ARRAYs.

.. note::
   **Unified API with Driver-Specific Entry Points**: The RelationalDBX provides a unified API for creating STRUCT and ARRAY values through common interfaces (``RelationalStruct``, ``RelationalArray``, ``RelationalStructBuilder``, ``RelationalArrayBuilder``). However, you must use the appropriate factory method for your JDBC driver:
...
The RelationalDBX extends standard JDBC to support STRUCT types, which represent nested record structures similar to protobuf messages. STRUCTs allow you to model hierarchical data within your relational schema.
...
The RelationalDBX supports ARRAY types containing elements of any SQL type, including primitives, STRUCTs, and even nested ARRAYs.
