...
Welcome to RelationalDBX Frequently Asked Questions (FAQ)! 

## About the RelationalDBX

### What is the RelationalDBX?

The RelationalDBX is a Java API that provides a structured, record-oriented store 
on top of the core FoundationDB store. For feature highlights, see the
[RelationalDBX overview](Overview.md). See the [getting started guide](GettingStarted.md)
...
The RelationalDBX defines a concept of a record store. A record store
...
Additionally the RelationalDBX allows for nested relationships such
...
databases are not available in the RelationalDBX, such as arbitrary 
aggregation, sorting, and complex joins.

Among the driving design principles of the RelationalDBX are support for
...
The RelationalDBX is, however, certainly intended as a foundation upon which
...
The RelationalDBX will assign a unique key value for each record type. In addition to avoiding
the problem of collisions with the rest of the primary key values, the RelationalDBX
...
automatically assigned by the RelationalDBX as an auto-incrementing value?
...
Long answer: The RelationalDBX does not currently offer any kind of auto-generated primary key.
...
No. Currently the RelationalDBX has no query language, however it does expose a query API 
and a query planner. In the future it is possible that the RelationalDBX may develop a
...
natural ordering of the primary keys or by indexes. The RelationalDBX has no facility 
...
your work into a number of smaller transactions. All streaming operations in the RelationalDBX
