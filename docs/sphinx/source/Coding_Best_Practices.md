# RelationalDBX Coding Best Practices
...
Almost all `Exception`s thrown in RelationalDBX extend `RecordCoreException`, and
...
Logging in RelationalDBX follows a similar paradigm to that described in 
...
In the RelationalDBX unit tests (any test extending `FDBTestBase`), the `asyncToSync()`
...
Otherwise it is executed in the global thread pool, rather than the RelationalDBX 
thread pool.
