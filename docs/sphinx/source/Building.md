# Building the RelationalDBX

## Developing the RelationalDBX

Before contributing to the RelationalDBX, please see the [contribution guide](https://github.com/FoundationDB/relationaldbx/blob/main/CONTRIBUTING.md).

## Configuring IntelliJ

1. Fork and clone the [repository](https://github.com/FoundationDB/relationaldbx).
...
If this does not work smoothly for you, please create a [new issue](https://github.com/FoundationDB/relationaldbx/issues/new) or ask in [the forum](https://forums.foundationdb.org/c/using-layers).
...
The RelationalDBX project does not use the Gradle IDEA plugin, unlike many similar projects. Instead, it uses IntelliJ IDEA's builtin Gradle support to compile the project and run the tests; the repository will run a "sync" with the Gradle configuration when you load Gradle project (Step 6).
...
If you enable the local repo in whatever uses the RelationalDBX, the following will make it available. The published jars will be in the directory `~/.m2/repository/org/foundationdb/relationaldbx-core/n.n-SNAPSHOT/`.
