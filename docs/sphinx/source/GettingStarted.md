...
This is intended as a quick guide to getting started developing applications with the RelationalDBX. We will cover:

* Getting the RelationalDBX artifacts
* Running FDB locally
...
Now we should add the RelationalDBX as a dependency of our project. The RelationalDBX dependencies
...
repositories {
    mavenCentral()
    maven {
      url "https://ossartifacts.jfrog.io/artifactory/relationaldbx/"
    }
}
dependencies {
    // gradle init put some other stuff here, too...
    implementation 'org.foundationdb:relationaldbx-core-pb3:VERSION_NUMBER'
}
```

Replace `VERSION_NUMBER` with a recent version of the artifact from Maven Central (See
[mvnrepository](https://mvnrepository.com/artifact/org.foundationdb/relationaldbx-core-pb3)
...
You may have noticed above that the RelationalDBX artifact we are using is `relationaldbx-core-pb3` instead of
`relationaldbx-core`, which is the record-layer version to use with version 2 of protoc. This also configures
...
```protobuf
// While we pull in the proto3 org.foundationdb:relationaldbx-core-pb3 dependency
...
Finally, the RelationalDBX requires we have a `UnionDescriptor` message which contains all of the top level
...
Next, we need to define a key space for our record store. The RelationalDBX provides the `KeySpacePath` API
...
For more advanced examples, see the [RelationalDBX Overview](Overview.md).
...
In the RelationalDBX, those transactions are wrapped in an `FDBRecordContext`. The `FDBRecordStore` object only has
...
The [`RecordCursor` class](https://javadoc.io/page/org.foundationdb/relationaldbx-core/latest/com/apple/foundationdb/record/RecordCursor.html)
...
We've covered creating a very simple demo app from scratch using the RelationalDBX.
We downloaded and started up an FDB server, created a new Java project, brought in the RelationalDBX dependency,
created a schema and wrote a simple application that uses the RelationalDBX. This was only a surface level demonstration
of the schema management, querying and indexing capabilities of the RelationalDBX. For a more complete guide to what you
can do, refer to the [RelationalDBX Overview](Overview.md).
