...
In practice, that means that indexes are translated into a RelationalDBX key expression and index type. Indexes currently
...
This index will contain two-tuples of `(fname, lname)`, but ordered only by `fname`. In the RelationalDBX, this
...
Indexes have a close relationship with RelationalDBX key expressions and index types. When the user defines a index
