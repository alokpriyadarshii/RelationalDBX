...
Joins combine rows from two or more tables based on related column. RelationalDBX supports INNER JOIN and joins using comma-separated table references in the FROM clause with join conditions specified in the WHERE clause.

.. important::

   RelationalDBX **supports** only one standard SQL JOIN keyword ``INNER JOIN`` (or just ``JOIN``).

   RelationalDBX **does not support** other standard SQL JOIN keywords (``LEFT JOIN``, ``RIGHT JOIN``, ``OUTER JOIN``, etc.). Use the comma-separated FROM clause instead.
...
RelationalDBX **does not support** ``CROSS JOIN`` keyword.
