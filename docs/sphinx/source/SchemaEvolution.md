...
If it is possible to know both the previous and new values for the meta-data, one can validate that the new meta-data has been responsibly evolved from the old meta-data by using the [`MetaDataEvolutionValidator`](https://javadoc.io/page/org.foundationdb/relationaldbx-core/latest/com/apple/foundationdb/record/metadata/MetaDataEvolutionValidator.html) class. Basic usage is something like:
...
Because of how Protobuf works, all existing records will have the new field but uninitialized. So this is a relatively safe operation. Note however that by default, the RelationalDBX will log a warning if it deserializes a Protobuf message with unknown fields, and application code that marshals Protobuf messages to their own types can easily drop these fields accidentally. Therefore, some care must be taken to make sure that before any client begins writing to that field that all clients have been updated to use the new meta-data.

### Remove a field from a record type

Rather than actually removing, it is better if the field is marked as deprecated and ignored by the application. If the field is removed, the RelationalDBX will print a warning when a record with that field is loaded, "Deserialized message has unknown fields."
