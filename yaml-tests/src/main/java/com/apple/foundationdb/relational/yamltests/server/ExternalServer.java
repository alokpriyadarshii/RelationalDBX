...
public class ExternalServer implements Clusters.BoundToCluster {
    ...
    public void start(Set<Integer> unavailablePorts) throws Exception {
        ...
        if (saveServerLogs) {
            ...
            outFile = File.createTempFile("relationaldbx-relational-server-" + version + "-" + currentTime + "-" + grpcPort + "-out.", ".log");
            errFile = File.createTempFile("relationaldbx-relational-server-" + version + "-" + currentTime + "-" + grpcPort + "-err.", ".log");
        } else {
            outFile = null;
            errFile = null;
        }
        ...
        try (JarFile jarFile = new JarFile(jar)) {
            ...
            if (version != null) {
                if (version.equals(BuildVersion.getInstance().getVersion())) {
                    ...
                    // See: https://github.com/FoundationDB/relationaldbx/issues/3208 for more details
