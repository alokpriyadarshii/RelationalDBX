...
/**
 * A value and its associated {@link RecordLayerPropertyKey} for a property that adopter configures for RelationalDBX.
 * Adopters can populate this value with providing a {@link #valueSupplier}.
 * The {@link #key} and its name, type and default value are supposed to completely defined by RelationalDBX.
 * The value is lazily loaded from the supplier when the property is read.
 * If no supplier is provided, a default value is used.
 * If the default value defined by RelationalDBX is supposed to be used, no need to put it into {@link RecordLayerPropertyStorage},
 * RelationalDBX can automatically read its default one.
 *
