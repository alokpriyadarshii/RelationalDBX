 ...
 * in control of {@code toProto(...)} which also happens to be implemented within the RelationalDBX class structure.
 * Therefore, we can easily instantiate the correct proto class and populate its fields. However, the opposite is not
 * true. If we were to call a {@code fromProto()} method directly we may know the exact RelationalDBX class and could
 * potentially instantiate that class, however, a lot of RelationalDBX classes implement a common interface
 ...
 * <br>
 * The example we use here is for the RelationalDBX interface
